...
Note that since the singletons of the constraint are actual variables of the MIP, an update of the singletons requires to update also the current objective value $c^Tx$. Finally, observe that the slacks are updated only after a successfull rounding or shifting, therefore $\Delta_{i}^{slack}$ can always be distributed correctly.
