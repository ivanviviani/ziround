...
data <- read.csv(paste("test_results_nogap(pre_",testype,").csv",sep=""), header=T, sep=";")
...
write.table(data, file=paste("test_results(pre_",testype,").csv",sep=""), row.names=FALSE, dec=".", sep=";", quote=FALSE)
...
write.table(df, file=paste("aggregate_measures(pre_",testype,").csv",sep=""), row.names=FALSE, col.names=FALSE, dec=".", sep=";", quote=FALSE)


#! Print aggregate measures to video
print(paste("Test type: pre_", testype, sep=""), quote=FALSE)
